...
const Cards = () => {
        const cards = [
            ...
            { title: "JOINED DATE", number: 1204, icon: SlCalender, bg: "orange-200/50", color: "orange-600" },
        ]
  return (
    <div className='w-full flex items-center justify-between gap-4 my-4'>
        {
            cards.map(({title, number, icon: Icon, bg, color}) => (
                <div className='flex flex-col gap-4 bg-white rounded-lg items-start p-4 w-full' key={title}>
                    <div className={`bg-${bg} p-3 rounded-full`}>
                        {Icon && <Icon className={`text-${color} text-2xl`} />}
                    </div>
                    <p>{title}</p>
                    <p className='text-md font-medium text-gray-700'>{number}</p>
